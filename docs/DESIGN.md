...
| `eggs` | `on` / `off` | The in-draft moments (§7.3): first kicker, a manager holding more than one kicker, a new highest sale, a finished roster. **The kill switch** — a full-screen overlay covers the board, and the roster-full one covers it for 15s, so it has to be switchable from a phone in the moment rather than by a deploy |
